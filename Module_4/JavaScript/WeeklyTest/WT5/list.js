...
window.addEventListener("load", () => {
  ...
  (async function listFetch() {
    ...
    list.innerHTML += `<p class = "list_item" onclick = "display_all()">All Categories</p>`;
    data.forEach((element) => {
      let elementJSON = JSON.stringify(element).replace(/"/g, "&quot;");
      list.innerHTML += `<p class ="list_item" onclick="display('${elementJSON}')">${element.list_name}</p>`;
    });
  })();
});
...
function showCardDetails(img, author, title) {
  let cardDetails = document.querySelector(".card-detail");
  ...
  let closeBtn = cardDetails.querySelector(".close-card-detail");
